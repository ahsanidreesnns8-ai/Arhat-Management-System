...
public class StockService {
    ...
    @Transactional
    public void decrementForSale(Long productId, BigDecimal qty, Long saleId, String notes) {
    ...
    @Transactional
    public void incrementForIncoming(Long productId, BigDecimal qty, Long referenceId, String notes) {
        Product product = productRepository.findById(productId)
                .orElseThrow(() -> new RuntimeException("Product not found"));
        ...
        BigDecimal newQty = previous.add(qty);
        stock.setQuantity(newQty);
        updateLowStockAlert(stock);
        stockRepository.save(stock);
        ...
        StockTransaction tx = StockTransaction.builder()
                .product(product)
                .transactionType(StockTransactionType.INCOMING)
                .quantity(qty)
                .previousQuantity(previous)
                .newQuantity(newQty)
                .notes(notes)
