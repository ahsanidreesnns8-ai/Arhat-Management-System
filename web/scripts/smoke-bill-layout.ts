...
async function main() {
  await runWithWorkspace('demo', async () => {
    ...
    try {
      ...
      assert(urduHtml.includes('بوری کلو'), 'Urdu farmer bill must use بوری کلو')
      ...
      assert(html.includes('>Bag<') || html.includes('>Bag</th>'), 'farmer bill missing bag kg column')
      assert(html.includes('>Bags<') || html.includes('<th>Bags</th>') || html.includes('>Bags</th>'), 'farmer bill missing Bags column')
      assert(html.includes('Extra'), 'farmer bill missing Extra kg column beside bags')
      assert(html.includes('>Man<') || html.includes('<th>Man</th>') || html.includes('>Man</th>'), 'farmer bill missing Man column')
      assert(html.includes('>Kg<') || html.includes('<th>Kg</th>') || html.includes('>Kg</th>'), 'farmer bill missing leftover kg column beside man')
      ...
      assert(html.includes('class="mark"'), 'farmer bill missing product letter column')
      assert(html.includes('W Wheat'), 'farmer bill missing product key')
      assert(!html.includes('<td class="mark">Wheat</td>'), 'farmer bill still prints full Wheat name')
      assert(!html.includes('Qty of one bag'), 'old long qty header still present')
      ...
      assert(buyerHtml.includes('>Bag<') || buyerHtml.includes('Bag'), 'buyer bill missing bag kg column')
