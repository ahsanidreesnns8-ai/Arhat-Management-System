...
async function main() {
  await runWithWorkspace('demo', async () => {
    ...
    try {
      ...
      assert(html.includes('Father name'), 'farmer bill missing father name')
      ...
      assert(buyerHtml.includes('<strong>Day</strong>'), 'buyer bill missing Day')
      assert(buyerHtml.includes('<strong>Time</strong>'), 'buyer bill missing Time')
