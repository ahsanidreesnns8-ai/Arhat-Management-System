...
async function main() {
  await runWithWorkspace('demo', async () => {
    ...
    try {
      ...
      assert(html.includes('>Bags<') || html.includes('<th>Bags</th>'), 'farmer bill missing Bags column')
      assert(html.includes('Extra kg'), 'farmer bill missing Extra kg column beside bags')
      assert(html.includes('>Man<') || html.includes('<th>Man</th>'), 'farmer bill missing Man column')
      assert(html.includes('>Kg<') || html.includes('<th>Kg</th>'), 'farmer bill missing leftover kg column beside man')
