...
async function main() {
  await runWithWorkspace('demo', async () => {
    ...
    try {
      ...
      assert(html.includes('aria-label="AI"'), 'farmer bill missing AI logo')
      ...
      assert(html.includes('size:5in 8in') || html.includes('size: 5in 8in'), 'farmer bill missing 5x8 page size')
      ...
      assert(html.includes('font-size:7.5px') || html.includes('font-size:7.2px'), 'farmer bill table font should be one size larger')
      assert(html.includes('text-align:center'), 'farmer bill table cells should be centered')
      assert(!html.includes('text-align:right') && !html.includes('text-align:left'), 'farmer bill table values should not sit on one side')
      ...
      assert(!buyerHtml.includes('This bill includes stock bags'), 'full buyer bill must not use the stock footer line')
      assert(!buyerHtml.includes('stock-note'), 'full buyer bill must not print a stock footer box')
      ...
      assert(!stockHtml.includes('This bill includes stock bags'), 'selected buyer bill must not use the stock footer line')
      assert(stockHtml.includes('002'), 'selected buyer bill should include 2 stock bags in the table')
      ...
      assert(!urduStock.includes('اس بل میں اسٹاک بوریاں شامل ہیں'), 'Urdu buyer bill must not use the stock footer line')
