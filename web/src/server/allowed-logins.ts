...
export const SHARED_SHOP_USERNAMES = ['owner', 'hasham'] as const

/** Retired shop usernames that must not be recreated. */
export const RETIRED_SHOP_USERNAMES = ['staff'] as const
...
export function isReservedLoginUsername(username: string) {
  const normalized = normalizeLoginUsername(username)
  return (
    isSharedShopLogin(normalized) ||
    (RETIRED_SHOP_USERNAMES as readonly string[]).includes(normalized)
  )
}
